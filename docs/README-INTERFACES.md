...
For the first setup, use [`README-INTRO.md`](./README-INTRO.md). For mode-specific page formats, use [`README-PHP.md`](./README-PHP.md), [`README-MD.md`](./README-MD.md) and [`README-RICH.md`](./README-RICH.md).
...
| Site texts | `$GLOBALS['zlangs']` | site title, subtitle, navigation labels and footer labels |
...
| ZTOKENS / footer tokens | `[@page]`, `[[@page]]`, `[footertext]`, `[VAR:name]` | replacement tokens for footer configuration and editor content |
...
Footer syntax is mode-independent. It works in PHP mode, MD mode and Rich mode because footer text is resolved by the common runtime footer logic. Footer tokens are part of ZTOKENS.
...
for translated footer text from `$GLOBALS['zlangs'][language]['foot']`.
...
`$GLOBALS['zlangs']` contains translated site texts.
...
MD additions/extensions for ZANACMS are the MD-mode additions to normal Markdown. They include internal `@page` links and compact image syntax for relative `/img` images. They are documented in [`README-MD.md`](./README-MD.md).
...
The HTML layout generator replaces documented placeholders in `design.html`. These HTML template replacement variables are called HVARS. Common HVARS are `~~ZHEAD~~`, `~~ZTITLE~~`, `~~ZH1~~`, `~~ZBODY~~`, `~~ZNAVI~~`, `~~ZFOOT~~` and `~~ZBASEURL~~`. The full list is documented in [`layout/html/README.md`](../layout/html/README.md). Framework CSS, JavaScript, fonts and images are referenced directly from `design.html` with `~~ZBASEURL~~`.
