...
This guide explains the basic file structure and setup of ZANACMS. For license terms, see [`LICENSE`](../LICENSE) and [`LICENSE-FAQ.md`](./LICENSE-FAQ.md). Bundled third-party components and notes about supported external design packages are listed in [`THIRD-PARTY-NOTICES.md`](../layout/THIRD-PARTY-NOTICES.md).
...
| Question | Document |
| --- | --- |
| I want to install ZANACMS | [`README.md`](../README.md), then this file |
| I want to choose the right mode | this file |
| I write PHP pages | [`README-PHP.md`](./README-PHP.md) |
| I write Markdown pages | [`README-MD.md`](./README-MD.md) |
| I use the Rich editor | [`README-RICH.md`](./README-RICH.md) |
| I need links, footer syntax, CSS, page variables or public functions | [`README-INTERFACES.md`](./README-INTERFACES.md) |
| I build or change layout generators | [`LAYOUT-GENERATOR-MANUAL.md`](./LAYOUT-GENERATOR-MANUAL.md) |
| I have practical licence questions | [`LICENSE-FAQ.md`](./LICENSE-FAQ.md) |

---

## Which mode should I choose?

Use [**PHP mode**](./README-PHP.md) if you want to write pages with PHP and arrays. Use [**MD mode**](./README-MD.md) if you want lightweight Markdown files with automatic cache building. Use [**Rich mode**](./README-RICH.md) if you want the normal visual editor workflow. In this file you can read about the shared parts. For public customisation and extension interfaces, see [`README-INTERFACES.md`](./README-INTERFACES.md).
...
The editor and media gallery are safety-disabled by default. Access can be enabled through the device manager `/admin/_d.php` for single devices/browsers. PHP mode does not need `/admin/` for runtime; it needs `/admin/` only if image upload through the optional media gallery or device access tools are used. MD mode needs `/admin/` only when the built-in Markdown editor or media tools are used. If Markdown files are edited directly, for example via FTP, `/admin/` can be removed and MD runtime still rebuilds `/zpcache/`. Rich mode needs `/admin/`; without the Rich editor, the Rich mode concept is not useful. 
...
Read the PHP mode guide for details: [`README-PHP.md`](./README-PHP.md)
...
Read the MD mode guide for details: [`README-MD.md`](./README-MD.md)
...
Read the Rich mode guide for details: [`README-RICH.md`](./README-RICH.md)
...
* [`README-PHP.md`](./README-PHP.md) for PHP mode
* [`README-MD.md`](./README-MD.md) for MD mode
* [`README-RICH.md`](./README-RICH.md) for Rich mode
* [`README-INTERFACES.md`](./README-INTERFACES.md) for public customisation and extension interfaces
...
The device manager (setup helper) is `/admin/_d.php`, and it is locked by `/__config/EDITACCESS_ENABLE.php`. You can use `/__config/EDITACCESS_ENABLE.php` to open a short setup period during which editing cookies can be created or deleted.
...
The editor and gallery are not shown if the cookie data is missing, has no current access entry, or the matching entry has expired. The edit cookie data is stored in `/zp/__zdata.php`.
...
If cached HTML output must be rebuilt manually, use `/admin/_d.php` and choose “Clear all HTML cache”.
...
Cached templates use the central placeholders from `ZP_HTML_CACHE_VARS`. `~~ZBASEURL~~` is the explicit basepath placeholder for local layout resources such as CSS, JavaScript and images. The full central placeholder list is in [`LAYOUT-GENERATOR-MANUAL.md`](./LAYOUT-GENERATOR-MANUAL.md#html-cache-standard).
...
These files are required for classic WordPress themes that expect WordPress to provide jQuery. See [`THIRD-PARTY-NOTICES.md`](../layout/THIRD-PARTY-NOTICES.md) for license information about the bundled script files and notes about external design packages.
...
ZANACMS should be customised through documented interfaces, not by changing core files. The detailed reference is [`README-INTERFACES.md`](./README-INTERFACES.md). The most important points are kept here because they affect normal setup work.
...
For complete details, including all parameters of `zlink()`, `zhref()`, `ztokenhref()`, `zautop()`, `zsetlangs()`, `zautolg()`, `zquote()`, `zoutimg()` and `out_page()`, see [`README-INTERFACES.md`](./README-INTERFACES.md).
