...
MD mode is the Markdown-based content mode of ZANACMS. It is for users who want to edit page files as Markdown, either directly or through the optional built-in Markdown editor. Editor-oriented Rich sites should use Rich mode; see [`README-RICH.md`](./README-RICH.md).

Shared setup topics such as layouts, languages, footer syntax, CSS and media access are in [`README-INTRO.md`](./README-INTRO.md). Public interface details, including ZTOKENS, page variables and footer tokens, are in [`README-INTERFACES.md`](./README-INTERFACES.md).
...
`pages` contains Markdown files and the navigation source file. The bundled starter files are in `/pages/example/md/`; copy the contents of that directory into `/pages/`, not the directory itself. After copying, the files should be placed directly as `/pages/index.en.md`, `/pages/contact.en.md` and `/pages/__NAVIGATION.txt`. `zpcache` contains generated MD cache files and should not be edited manually. `/admin/` is only needed when the built-in editor or media tools are used; it can be removed when Markdown files are edited directly, for example via FTP. MD runtime, parser, navigation handling, cache rebuilding and HTML sanitation stay in `/zp/md.php`, because newly uploaded Markdown files still have to be parsed and converted into cache files during normal runtime.
...
The same internal-link idea is used in Rich mode and is described in more detail in [`README-RICH.md`](./README-RICH.md#8-internal-links).
...
They are intended for prepared reusable values or generated output. A PHP developer can provide the variable, and a normal editor user can insert it without writing PHP. More detail is in [`README-INTERFACES.md`](./README-INTERFACES.md#8-page-variables).
...
See [`README-RICH.md`](./README-RICH.md#9-variables) for the common variable rules.
...
URL rewrite is optional and off by default. With `urlrewrite`, internal MD links such as `[Contact](@contact)` use the configured URL base. With rewrite enabled they are delivered as URLs such as `/en/contact`. The MD page cache writes dynamic `zlink()` calls for page output, so the current language, base path and `lay` design selector are applied when the cached page is included. The MD meta cache is different: `_meta.php` is a neutral data cache and keeps internal footer/page references as tokens instead of dynamic `zlink()` calls.
...
MD uses the meta cache to know which languages exist for each page. Official website languages come from `$GLOBALS['zlangs']`. Additional MD files outside the official website languages add their language only on that page. If a non-official language is requested and the page does not have that language, MD delivers `fallbacklg` or `stdlang`; canonical and further internal links use that delivered language. If an official language is requested but the current page is missing in that language, MD still keeps further internal links in the requested official language, while canonical points to the delivered fallback language.

The cache exists so Markdown files do not need to be parsed on every page request. ZP rebuilds cache files when the Markdown source file, `__NAVIGATION.txt` or `/__config/conf.php` has changed, or when a required cache file is missing. This also applies when Markdown files are uploaded or edited by FTP. MD language availability is stored in the meta cache as `langs`; its default lifetime is 300 seconds and can be overridden with `mdlangcachetime`, for example `// 'mdlangcachetime'=>3600, // 1h MD langs cache time`. With an admin cookie, MD language availability uses a short 60 second cache window, so new FTP files become visible during editing without rebuilding the meta cache on every request. To see real public website build times, test in another browser or without the admin cookie. If generated output looks stale or broken, delete the files in `/zpcache`; ZP can rebuild them.
...
- [`README-INTRO.md`](./README-INTRO.md) for shared setup and all-modes topics
- [`README-INTERFACES.md`](./README-INTERFACES.md) for ZTOKENS, footer tokens, page variables and public interfaces
- [`README-RICH.md`](./README-RICH.md) for the visual Rich editor workflow
